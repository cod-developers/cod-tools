/*!  \file sfcalc_obs.h
  Header file for structure factor calculation vs observed object
*/
...
#ifndef CLIPPER_SFCALC_OBS
#define CLIPPER_SFCALC_OBS
...
namespace clipper {
  ...
  /*! Perform structure factor calculation, adding an additional bulk
    ...
    \ingroup g_funcobj */
  template<class T> class SFcalc_obs_bulk : public SFcalc_obs_base<T> {
  public:
    //! constructor
    SFcalc_obs_bulk( const int n_params = 12 ) : nparams(n_params) {}
    //! constructor: shorthand for constructor+operator
    SFcalc_obs_bulk( HKL_data<datatypes::F_phi<T> >& fphi, const HKL_data<datatypes::F_sigF<T> >& fsig, const Atom_list& atoms, const int n_params = 12 ) : nparams(n_params) { (*this)( fphi, fsig, atoms ); }
