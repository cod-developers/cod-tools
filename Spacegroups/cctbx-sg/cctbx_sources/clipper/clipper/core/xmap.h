...
#ifndef CLIPPER_XMAP2
#define CLIPPER_XMAP2
...
namespace clipper
{
  ...
  {
  public:
    Xmap_cachekey( const Spgr_descr& spgr_descr, const Grid_sampling& grid ) :
  ...
  {
  public:
    Xmap_cacheobj( const Xmap_cachekey& xmap_cachekey );  //!< construct entry
    bool matches( const Xmap_cachekey& xmap_cachekey ) const; //!< compare entry
    String format() const;  //!< string description
    // data
    Xmap_cachekey key;               //!< key
  ...
  */
  ...
  public:
    ...
    bool in_map( const Coord_grid& pos ) const { return true; }
    //! (This method is for compatibility with NXmap - it always returns true)
  ...
  */
  ...
  namespace data
  {
    extern ObjectCache<Xmap_cacheobj> XMcache;
  }
