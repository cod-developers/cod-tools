...
namespace clipper {
  ...
  */
  ...
  // ObjectCache key type
  typedef Spgr_descr Spgr_cachekey;

  // ObjectCache data type
  class Spgr_cacheobj
  {
  public:
    Spgr_cacheobj( const Spgr_cachekey& spgr_cachekey );  //!< construct entry
    bool matches( const Spgr_cachekey& spgr_cachekey ) const; //!< compare entry
    String format() const;  //!< string description
    // data
    Spgr_cachekey spgr_cachekey_;       //!< spacegroup cachekey
  ...
  */
  ...
  namespace data
  {
    extern ObjectCache<Spgr_cacheobj> SGcache;
  }
