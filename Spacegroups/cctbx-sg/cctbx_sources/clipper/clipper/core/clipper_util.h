...
namespace clipper
{
  ...
  public:
    ...
    inline static void set_null( ftype32& f ) { *((uitype32*)&f) = *((const uitype32*)&nanf_); }
    //! set null floating value - a specific value of NaN used for missings
    inline static void set_null( ftype64& f ) { *((uitype64*)&f) = *((const uitype64*)&nand_); }
    //! fast test for null floating value - only works if set from Util::null()
    inline static bool is_null( const ftype32& f ) { return ( *((const uitype32*)&f) == *((const uitype32*)&nanf_) ); }
    //! fast test for null floating value - only works if set from Util::null()
    inline static bool is_null( const ftype64& f ) { return ( *((const uitype64*)&f) == *((const uitype64*)&nand_) ); }
    //! fast Util::nan() test
    /*! Used for missing entries: THIS DOES NOT DISTINGUISH BETWEEN NAN & INF */
    inline static bool is_nan( const ftype32 f ) { return ((*((const uitype32*)&f)&CLIPPER_NAN_MASK_A_32)==CLIPPER_NAN_MASK_A_32); }
    //! fast Util::nan() test
    /*! Used for missing entries: THIS DOES NOT DISTINGUISH BETWEEN NAN & INF */
    inline static bool is_nan( const ftype64 f ) { return ((*((const uitype64*)&f)&CLIPPER_NAN_MASK_A_64)==CLIPPER_NAN_MASK_A_64); }
    //! slow general NaN test for compatibility
    /*! Works for all architectures with IEEE arithmetic only */
    inline static bool isnan(const ftype32 f) { return ((*((const uitype32*)&f)&CLIPPER_NAN_MASK_A_32)==CLIPPER_NAN_MASK_A_32)&&((*((const uitype32*)&f)&CLIPPER_NAN_MASK_B_32)!=0U); }
    //! slow general NaN test for compatibility
    /*! Works for all architectures with IEEE arithmetic only */
    inline static bool isnan(const ftype64 f) { return ((*((const uitype64*)&f)&CLIPPER_NAN_MASK_A_64)==CLIPPER_NAN_MASK_A_64)&&((*((const uitype64*)&f)&CLIPPER_NAN_MASK_B_64)!=0U); }
