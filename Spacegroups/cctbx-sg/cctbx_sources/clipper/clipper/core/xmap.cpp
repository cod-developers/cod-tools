...
Xmap_cacheobj::Xmap_cacheobj( const Xmap_cachekey& xmap_cachekey ) :
  key( xmap_cachekey )
{
...
bool Xmap_cacheobj::matches( const Xmap_cachekey& xmap_cachekey ) const
{
  return
...
void Xmap_base::init( const Spacegroup& spacegroup, const Cell& cell, const Grid_sampling& grid_sam )
{
  ...
  Xmap_cachekey key( spacegroup, grid_sam );
  cacheref = data::XMcache.cache( key );
...
void Xmap_base::asu_error() const
{
...
namespace data { ObjectCache<Xmap_cacheobj> XMcache; }

//template ObjectCache<Xmap_cacheobj>::Reference ObjectCache<Xmap_cacheobj>::cache<Xmap_cachekey>( const Xmap_cachekey& key );
